...
export async function POST(req: NextRequest) {
  ...
  if (req.method === 'POST') {
    const { nome, cpf, nascimento, email, tel, cep, logradouro, bairro, cidade, estado, placa, modelo, ano, km, sintomas } = request;
    ...
    try {
      ...
      await conn.commit();
      return NextResponse.json({ message: 'Dados inseridos com sucesso!' }, {status:201});
    } catch (error) {
      console.error(error);
      return NextResponse.json({ error: 'Erro ao inserir dados.' }, {status:400});
    }
  } else {
    return NextResponse.json({ message: 'Método não permitido' }, {status:403});
  }
}
